...
library(rstanarm) #bayesian mixed models package
...
```{r}
#Nitrate Only Data
PAM_nitrate <- PAM_all%>% filter(str_detect(treat_ID_full, "N"))

PAM_ammonium <- PAM_all%>% filter(str_detect(treat_ID_full, "A_"))
...
#https://en.wikipedia.org/wiki/Autoregressive_mode
# MCMC: Markov chain Monte Carlo -- samples from a probability distribution

nit_1_stanlmer <- stan_lmer(formula = PAM_delta ~ dose_type + dose_level + temp + food + sym_state + (1 | exp_num), data=PAM_nitrate, seed=349)
?stan_aov

#print our output
print(nit_1_stanlmer, digits = 2)

#Mean and standard deviation of the posteriur distribution
summary(nit_1_stanlmer, 
        pars = c("(Intercept)", "sigma", "Sigma[school:(Intercept),(Intercept)]"),
        probs = c(0.025, 0.975),
        digits = 2)
#it is using Gaussian

#BIC?
#add a jar_position here as a fixed effect
...
#least squares just minimizes the sum of squares aka how small residuals are
```
...
plot_coefs(lmer_PAM_delta_5)

summary(lmer_PAM_delta_5)

#MY BIG BEAAAAUTIFUL MODEL
lmer_PAM_delta_6 <-  lmer(PAM_delta ~ as.factor(dose_type) + as.factor(dose_level) + as.factor(dose_type)*as.factor(dose_level) + as.factor(dose_level)*as.factor(sym_state)+  as.factor(dose_type)*as.factor(sym_state)+ as.factor(temp)*as.factor(sym_state) + as.factor(temp)*as.factor(sym_state)*as.factor(dose_type)  + as.factor(temp)*as.factor(dose_type) + as.factor(temp) + as.factor(sym_state) + as.factor(food) + (1|col_num) + (1|jar_unique_ID) + (1|exp_run_full), data=PAM_all)
...
ammonium_all <- subset(PAM_all, PAM_all$N_species=="A" | PAM_all$N_species=="CNTRL")
nitrate_all <- subset(PAM_all, PAM_all$N_species=="N" | PAM_all$N_species=="CNTRL")

lmer_PAM_ammonium_6 <-  lmer(PAM_delta ~  as.factor(dose_level) +  as.factor(dose_level)*as.factor(sym_state) + as.factor(temp)*as.factor(sym_state) + as.factor(temp) + as.factor(sym_state) + as.factor(food) + (1|col_num) + (1|jar_unique_ID) + (1|exp_run_full), data=ammonium_all)
summary(lmer_PAM_ammonium_6)

lmer_PAM_nitrate_6 <-  lmer(PAM_delta ~  as.factor(dose_level) +  as.factor(dose_level)*as.factor(sym_state)+  as.factor(temp)*as.factor(sym_state) + as.factor(temp) + as.factor(sym_state) + as.factor(food) + (1|col_num) + (1|jar_unique_ID) + (1|exp_run_full), data=nitrate_all)
summary(lmer_PAM_nitrate_6)

# adding more dose type interactions
lmer_PAM_delta_7 <-  lmer(PAM_delta ~ as.factor(dose_type) + as.factor(dose_level) + as.factor(dose_type)*as.factor(dose_level) + as.factor(dose_level)*as.factor(sym_state)+  as.factor(dose_type)*as.factor(sym_state)+ as.factor(temp)*as.factor(sym_state) + as.factor(dose_type)*as.factor(food) + as.factor(temp)*as.factor(sym_state)*as.factor(dose_type)  + as.factor(temp)*as.factor(dose_type) + as.factor(temp) + as.factor(sym_state) + as.factor(food) + (1|col_num) + (1|jar_unique_ID) + (1|exp_run_full), data=PAM_all)
summary(lmer_PAM_delta_7)

lmer_PAM_delta_8 <-  lmer(PAM_delta ~ as.factor(dose_type) + as.factor(dose_level) + as.factor(food) + as.factor(temp) + as.factor(sym_state)  + as.factor(dose_type)*as.factor(dose_level) + as.factor(dose_type)*as.factor(dose_level)*as.factor(food) + as.factor(dose_type)*as.factor(dose_level)*as.factor(sym_state) + as.factor(dose_type)*as.factor(dose_level)*as.factor(temp) + (1|col_num) + (1|jar_unique_ID) + (1|exp_run_full), data=PAM_all)
                            
summary(lmer_PAM_delta_8)                           
                            
#if our goal is p values, then we need to use separate models
# if our goal 
