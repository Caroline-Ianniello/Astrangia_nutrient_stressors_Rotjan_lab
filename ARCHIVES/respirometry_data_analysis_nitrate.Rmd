---
title: "Respirometry data analysis (NITRATE) C. Fleming Last updated: 12/13/23"
output:
...
getwd()
resp_all_nitrate.data <- read.csv("C:/Users/carol/OneDrive/Documents/PhD Boston University 2019-/Astrangia_nutrient_stressors_Rotjan_lab/Data/RESP_ALL_SUMMARY_ALL_cleaned_dead_and_broken_outlierG28Y65_deleted_SA_CORR_5_24_24.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
#filter by anything that starts with the letter "A" in the treat_ID_full column
resp_all_nitrate.data <- resp_all_nitrate.data %>%
    filter(substr(resp_all_nitrate.data$treat_ID_full, 1, 1) == "N")

hist(log(resp_all_nitrate.data$abs_resp_rate_SA_corr)) #logging this looks GREAT
qqnorm(log(resp_all_nitrate.data$abs_resp_rate_SA_corr)) #this looks really nice

#create a natural logged version of the absolute value of resp rate
resp_all_nitrate.data$LOG_abs_resp_rate_SA_corr <- log(resp_all_nitrate.data$abs_resp_rate_SA_corr)
hist(resp_all_nitrate.data$LOG_abs_resp_rate_SA_corr)

#make sure treatID is a character
#resp_all_nitrate.data$treat_ID_full<-as.factor(resp_all_nitrate.data$treat_ID_full)
...
resp_all_nitrate.data$N_dose <- factor(resp_all_nitrate.data$N_dose, levels=c(0,1.5,5,18))
resp_all_nitrate.data$sym_state<- as.factor(resp_all_nitrate.data$sym_state)
sum_sym_state_nitrate <- summarySE(resp_all_nitrate.data, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose"))
# Visualize
gg_sym_state_nitrate<-ggplot(data = sum_sym_state_nitrate,
   ...
   scale_x_discrete("Nitrate Dose", labels=c("0uM","1.5uM","5uM","18uM"))+
    labs(x = "Nitrate Dose", 
...
# # NO E COLI AT 20C
resp_NOEC_20C <- resp_all_nitrate.data %>%
  filter(e_coli_plate==0 & temp==20)
resp_NOEC_20C$N_dose <- factor(resp_NOEC_20C$N_dose, levels=c(0,1.5,5,18))
resp_NOEC_20C$sym_state<- as.factor(resp_NOEC_20C$sym_state)
sum_nitrate_NOEC_20C<- summarySE(resp_NOEC_20C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose"))
# Visualize
...
# # E COLI AT 20C
resp_EC_20C <- resp_all_nitrate.data %>%
  filter(e_coli_plate==1 & temp==20)
resp_EC_20C$N_dose <- factor(resp_EC_20C$N_dose, levels=c(0,1.5,5,18))
resp_EC_20C$sym_state<- as.factor(resp_EC_20C$sym_state)
sum_nitrate_EC_20C<- summarySE(resp_EC_20C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose"))
# Visualize
gg_nitrate_EC_20C <-ggplot(data = sum_nitrate_EC_20C,
       aes(x = N_dose,
           y = abs_resp_rate_SA_corr, col=sym_state)) +
    geom_point() +
  geom_jitter(data = resp_EC_20C, aes(x = N_dose,
           y = abs_resp_rate_SA_corr, col=sym_state), position = position_dodge(.8)) +
    geom_errorbar(aes(ymin=abs_resp_rate_SA_corr-se, ymax=abs_resp_rate_SA_corr+se), width=0.35, size = 1) +
...
# # NO E COLI AT 30C
resp_NOEC_30C <- resp_all_nitrate.data %>%
  filter(e_coli_plate==0 & temp==30)
resp_NOEC_30C$N_dose <- factor(resp_NOEC_30C$N_dose, levels=c(0,1.5,5,18))
resp_NOEC_30C$sym_state<- as.factor(resp_NOEC_30C$sym_state)
sum_nitrate_NOEC_30C<- summarySE(resp_NOEC_30C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose"))
# Visualize
...
# # E COLI AT 30C
resp_EC_30C <- resp_all_nitrate.data %>%
  filter(e_coli_plate==1 & temp==30)
resp_EC_30C$N_dose <- factor(resp_EC_30C$N_dose, levels=c(0,1.5,5,18))
resp_EC_30C$sym_state<- as.factor(resp_EC_30C$sym_state)
sum_nitrate_EC_30C<- summarySE(resp_EC_30C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose"))
# Visualize
...
# # By temperature
resp_NOEC <- resp_all_nitrate.data %>%
  filter(e_coli_plate==0)
resp_NOEC$N_dose <- factor(resp_NOEC$N_dose, levels=c(0,1.5,5,18))
resp_NOEC$sym_state<- as.factor(resp_NOEC$sym_state)
resp_NOEC$temp<- as.factor(resp_NOEC$temp)
sum_nitrate_NOEC<- summarySE(resp_NOEC, measurevar = "abs_resp_rate_SA_corr", groupvars = c("N_dose", "temp"))
sum_nitrate_NOEC$N_dose <- factor(sum_nitrate_NOEC$N_dose, levels=c(0,1.5,5,18))
sum_nitrate_NOEC$temp<- as.factor(sum_nitrate_NOEC$temp)
# Visualize
...
gg_nitrate_NOEC 
#
resp_EC <- resp_all_nitrate.data %>%
  filter(e_coli_plate==1)
sum_nitrate_EC<- summarySE(resp_EC, measurevar = "abs_resp_rate_SA_corr", groupvars = c("N_dose", "temp"))
sum_nitrate_EC$N_dose <- factor(sum_nitrate_EC$N_dose, levels=c(0,1.5,5,18))
sum_nitrate_EC$temp<- as.factor(sum_nitrate_EC$temp)
...
```
# Sym state, dose, AND food

```{r}
resp_all_nitrate.data$N_dose <- factor(resp_all_nitrate.data$N_dose, levels=c(0,1.5,5,18))
resp_all_nitrate.data$sym_state<- as.factor(resp_all_nitrate.data$sym_state)
sum_sym_state_nitrate_food <- summarySE(resp_all_nitrate.data, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose", "food"))
# Visualize
gg_sym_state_nitrate_food<-ggplot(data = sum_sym_state_nitrate_food,
       aes(x = N_dose,
  ...
  scale_shape_discrete(labels=c("Fed","Starved"),name="Nutritional State") +
   ...
   scale_x_discrete("Nitrate Dose", labels=c("0uM","1.5uM","5uM","18uM"))+
    labs(x = "Nitrate Dose", 
       ...
       # theme(legend.position="none")+
  scale_color_manual("Symbiotic state", labels=c("Aposymbiotic","Symbiotic"), values=c("gray", "brown"))
gg_sym_state_nitrate_food


# # NO E COLI AT 20C
sum_nitrate_NOEC_20C_food<- summarySE(resp_NOEC_20C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose", "food"))
# Visualize
...
# # E COLI AT 20C
sum_nitrate_EC_20C_food<- summarySE(resp_EC_20C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose","food"))
# Visualize
...
# # NO E COLI AT 30C
sum_nitrate_NOEC_30C_food<- summarySE(resp_NOEC_30C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose","food"))
# Visualize
...
# # E COLI AT 30C
sum_nitrate_EC_30C_food<- summarySE(resp_EC_30C, measurevar = "abs_resp_rate_SA_corr", groupvars = c("sym_state", "N_dose","food"))
# Visualize
...
# # By temperature
sum_nitrate_NOEC_food<- summarySE(resp_NOEC, measurevar = "abs_resp_rate_SA_corr", groupvars = c("N_dose", "temp","food"))
sum_nitrate_NOEC_food$N_dose <- factor(sum_nitrate_NOEC_food$N_dose, levels=c(0,1.5,5,18))
sum_nitrate_NOEC_food$temp<- as.factor(sum_nitrate_NOEC_food$temp)
# Visualize
...
#
sum_nitrate_EC_food<- summarySE(resp_EC, measurevar = "abs_resp_rate_SA_corr", groupvars = c("N_dose", "temp","food"))
sum_nitrate_EC_food$N_dose <- factor(sum_nitrate_EC_food$N_dose, levels=c(0,1.5,5,18))
sum_nitrate_EC_food$temp<- as.factor(sum_nitrate_EC_food$temp)
gg_nitrate_EC_food <-ggplot(data = sum_nitrate_EC_food,
       aes(x = N_dose,
           y = abs_resp_rate_SA_corr, col=temp, shape=food)) +
   geom_point(size=5, position=position_dodge(width=0.5)) +
    geom_errorbar(aes(ymin=abs_resp_rate_SA_corr-se, ymax=abs_resp_rate_SA_corr+se), width=0.35, size = 1, position=position_dodge(width=0.5)) +
...
```{r for SICB}
#Fed, 20C, no E coli
SICB_data_nitrate_1 <- resp_all_nitrate.data %>%
  filter(food=="F") %>%
  filter(temp==20) %>%
  filter(abs_resp_rate_SA_corr<0.7) %>% #there is one 7.5 at 20C thats really high and looks like an outlier to me
  filter(beetag_exprun!= "G33Y97_exp2A") #this was at E coli at 30C, and it was at like 0.23 and also looked like an outlier

sum_N_species_dose_nitrate <- summarySE(SICB_data_nitrate_1, measurevar = "abs_resp_rate_SA_corr", groupvars = c("N_species_dose"))
ggplot(data = sum_N_species_dose_nitrate ,
       aes(x = N_species_dose,
           y = abs_resp_rate_SA_corr, fill="black")) +
    geom_col(fill=c("#587E9F","#71E379","#57A45D","#026109"), color="black", alpha=0.8, width=0.5) +
  ...
  labs(x = "Nitrate dose", 
         ...
         caption = "data represent mean +/- SEM" ) +
    scale_x_discrete("Nitrate dose", labels=c(CNTRL_0="0 uM", N_1.5="1.5 uM", N_5="5 uM", N_18="18 uM"))

nitrate_SICB_model_1 <- lmer(LOG_abs_resp_rate_SA_corr ~ N_species_dose + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate_1)
summary(nitrate_SICB_model_1)
(summary(emmeans(nitrate_SICB_model_1, specs=pairwise ~ N_species_dose)))

#fed, with temperature, NO E. coli
SICB_data_nitrate_noEC <- resp_all_nitrate.data %>%
  filter(food=="F") %>%
  filter(e_coli_plate==0) %>%
 filter(abs_resp_rate_SA_corr<0.7) %>% #there is one 7.5 at 20C thats really high and looks like an outlier to me
  filter(beetag_exprun!= "G33Y97_exp2A") #this was at E coli at 30C, and it was at like 0.23 and also looked like an outlier
nitrate_SICB_model_noEC <- lmer(LOG_abs_resp_rate_SA_corr ~ control_or_nutrient + temp + red_day11 +  control_or_nutrient*temp + control_or_nutrient*red_day11 + temp*red_day11 + control_or_nutrient*red_day11*temp + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate_noEC)
# boundary (singular) fit: see help('isSingular') HAPPENS WHEN RANDOM EFFECTS ARE VERY SMALL
plot_model(nitrate_SICB_model_noEC, vline.color="black")
summary(nitrate_SICB_model_noEC)
#nothing sig
(summary(emmeans(nitrate_SICB_model_noEC, specs=pairwise ~ temp)))
#again we see metabolic depression

sum_SICB_data_nitrate_noEC <- summarySE(SICB_data_nitrate_noEC, measurevar = "abs_resp_rate_SA_corr", groupvars = c("control_or_nutrient", "temp"))

sum_SICB_data_nitrate_noEC$temp <- as.factor(sum_SICB_data_nitrate_noEC$temp)

ggplot(data = sum_SICB_data_nitrate_noEC,
    ...
    labs(x = "Nitrate dose", 
         y = expression(paste("Rate O2 Consumption (umol*hr"^"-1"~"*cm"^"2"~")")), 
         caption = "data represent mean +/- SEM" ) +
    scale_x_discrete("", labels=c(Control="No Nitrate", Nitrate="Nitrate")) +
   theme(legend.position = "right",
          legend.title = element_text(size=12),
          axis.title = element_text(size = 12),
          legend.background = element_blank(),
...
count(SICB_data_nitrate_noEC$control_or_nutrient=="Control")

?geom_errorbar

#filter all fed, at all dose levels
SICB_data_nitrate <- resp_all_nitrate.data %>%
  filter(food=="F") %>%
  filter(abs_resp_rate_SA_corr<0.7) %>% #there is one 7.5 at 20C thats really high and looks like an outlier to me
  filter(beetag_exprun!= "G33Y97_exp2A") #this was at E coli at 30C, and it was at like 0.23 and also looked like an outlier

#create an lmer with this dataset
nitrate_SICB_model <- lmer(LOG_abs_resp_rate_SA_corr ~ control_or_nutrient + temp + red_day11 + e_coli_plate + control_or_nutrient*temp + control_or_nutrient*e_coli_plate + e_coli_plate*temp + e_coli_plate*red_day11 + control_or_nutrient*red_day11 + temp*red_day11 + control_or_nutrient*red_day11*temp + control_or_nutrient*temp*e_coli_plate + control_or_nutrient*red_day11*e_coli_plate + temp*red_day11*e_coli_plate + control_or_nutrient*red_day11*temp*e_coli_plate + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate)
tab_model(nitrate_SICB_model) #gives you a chart
plot_model(nitrate_SICB_model, vline="black")

summary(nitrate_SICB_model)
(summary(emmeans(nitrate_SICB_model, specs=pairwise ~ temp)))
(summary(emmeans(nitrate_SICB_model, specs=pairwise ~ e_coli_plate)))
(summary(emmeans(nitrate_SICB_model, specs=pairwise ~ control_or_nutrient)))
(summary(emmeans(nitrate_SICB_model, specs=pairwise ~ temp*e_coli_plate*control_or_nutrient)))

#TEMPERATURE IS SIGNFICIANT
library(ggplot2)

ggplot(data = SICB_data_nitrate ,
       aes(x = control_or_nutrient,
...
ggplot(data = SICB_data_nitrate_noEC,
       aes(x = temp,
...
nitrate_SICB_model_RGB <- lmer(red_day11 ~ control_or_nutrient + temp + sym_state + control_or_nutrient*temp + control_or_nutrient*sym_state + temp*sym_state + control_or_nutrient*sym_state*temp  + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate)
summary(nitrate_SICB_model_RGB)
(summary(emmeans(nitrate_SICB_model_RGB, specs=pairwise ~ control_or_nutrient))) 
...
ggplot(data = SICB_data_nitrate ,
  ...
  labs(x = "Nitrate dose", 
    ...
    scale_x_discrete("", labels=c(Control="No Nitrate", nitrate="Nitrate")) +
    scale_color_manual("Temperature", labels=c("20°C", "30°C"),values=c("blue", "darkred")) 
#NO RELATIONSHIP BETWEEN COLOR AND RESP RATE IN NITRATE (not all that surprising)
...
# Nitrate
summary(aov_dose<-aov(resp_rate ~ N_species_dose, data=e_coli))
...
# STATS STATS STATS
lm_resp_control_v_nutrient <- lm(resp_rate ~  temp + e_coli_plate + control_or_nutrient + (1 | col_num), data=resp_all_nitrate.data)
