...
#What questions am I answering with this PCA?

#1) Are there similarities in the response of a single individual in their respiration rate, change in R channel value, and change in photosynthetic efficiency?
#2) Can we characterize groups of individuals by variables?
...
#susbet out beetag_exprun because it causes some problems
ammonium_all_data <- subset(ammonium_all_data, select = -c(beetag_exprun) )


#lets make a new dataset that omits NAs because if we don't have all the data then the PCA function freaks
ammonium_all_data <-  na.omit(ammonium_all_data)
```

```{r Pretty PCAs of all independent variables}
# Using ggord package to make PCAs
ord_ammonium <- PCA(ammonium_all_data[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state <- ggord(ord_ammonium, ammonium_all_data$sym_state)
pca_sym_state

#PCA by dose
ammonium_all_data$dose<-as.character(ammonium_all_data$dose)
pca_dose <- ggord(ord_ammonium, ammonium_all_data$dose)
pca_dose

#PCA by temperature
ammonium_all_data$temp<-as.character(ammonium_all_data$temp)
pca_temp <- ggord(ord_ammonium, ammonium_all_data$temp)
pca_temp

#PCA by treatment ID
pca_treat_ID <- ggord(ord_ammonium, ammonium_all_data$treat_ID)
pca_treat_ID
...
```{r E. coli data PCAs}
#subset data that was treated with E. coli
ammonium_all_ecoli <- subset(ammonium_all_data, ammonium_all_data$e_coli_plate==1)

# Using ggord package to make PCAs
ord_ammonium <- PCA(ammonium_all_ecoli[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_ecoli <- ggord(ord_ammonium, ammonium_all_ecoli$sym_state)
pca_sym_state_ecoli 

#PCA by dose
ammonium_all_ecoli$dose<-as.character(ammonium_all_ecoli$dose)
pca_dose_ecoli  <- ggord(ord_ammonium, ammonium_all_ecoli$dose)
pca_dose_ecoli 

#PCA by temperature
ammonium_all_ecoli$temp<-as.character(ammonium_all_ecoli$temp)
pca_temp_ecoli  <- ggord(ord_ammonium, ammonium_all_ecoli$temp)
pca_temp_ecoli 

#PCA by treatment ID
pca_treat_ID_ecoli  <- ggord(ord_ammonium, ammonium_all_ecoli$treat_ID)
pca_treat_ID_ecoli 
...
```{r No E. coli PCAs}
#subset data that was treated with E. coli
ammonium_all_no_ecoli <- subset(ammonium_all_data, ammonium_all_data$e_coli_plate==0)

# Using ggord package to make PCAs
ord_ammonium <- PCA(ammonium_all_no_ecoli[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_no_ecoli <- ggord(ord_ammonium, ammonium_all_no_ecoli$sym_state)
pca_sym_state_no_ecoli 

#PCA by dose
ammonium_all_no_ecoli$dose<-as.character(ammonium_all_no_ecoli$dose)
pca_dose_no_ecoli  <- ggord(ord_ammonium, ammonium_all_no_ecoli$dose)
pca_dose_no_ecoli 

#PCA by temperature
ammonium_all_no_ecoli$temp<-as.character(ammonium_all_no_ecoli$temp)
pca_temp_no_ecoli  <- ggord(ord_ammonium, ammonium_all_no_ecoli$temp)
pca_temp_no_ecoli 

#PCA by treatment ID
pca_treat_ID_no_ecoli  <- ggord(ord_ammonium, ammonium_all_no_ecoli$treat_ID)
pca_treat_ID_no_ecoli 

```


```{r Pretty PCAs of SYMBIONT HEALTH VARIABLES}
# Using ggord package to make PCAs
ord_ammonium_symbiont <- PCA(ammonium_all_data[, 2:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_symbiont <- ggord(ord_ammonium_symbiont, ammonium_all_data$sym_state)
pca_sym_state_symbiont

#PCA by dose
ammonium_all_data$dose<-as.character(ammonium_all_data$dose)
pca_dose_symbiont <- ggord(ord_ammonium_symbiont, ammonium_all_data$dose)
pca_dose_symbiont

#PCA by temperature
ammonium_all_data$temp<-as.character(ammonium_all_data$temp)
pca_temp_symbiont <- ggord(ord_ammonium_symbiont, ammonium_all_data$temp)
pca_temp_symbiont

#PCA by treatment ID
pca_treat_ID_symbiont <- ggord(ord_ammonium_symbiont, ammonium_all_data$treat_ID)
pca_treat_ID_symbiont

```


```{r Lets subset 7.5 and control to see whats up}
ammonium_control_7.5 <- subset(ammonium_all_data, ammonium_all_data$dose==0 | ammonium_all_data$dose==7.5)

ord_ammonium_7.5 <- PCA(ammonium_control_7.5[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_7.5 <- ggord(ord_ammonium_7.5, ammonium_control_7.5$sym_state)
pca_sym_state_7.5

#PCA by dose
ammonium_all_data$dose<-as.character(ammonium_control_7.5$dose)
pca_dose_7.5 <- ggord(ord_ammonium_7.5, ammonium_control_7.5$dose)
pca_dose_7.5


#nothing that striking, let's try with 7.5 and 10
ammonium_control_7.5_10 <- subset(ammonium_all_data, ammonium_all_data$dose==10 | ammonium_all_data$dose==7.5)
ord_ammonium_7.5_10 <- PCA(ammonium_control_7.5_10[, 1:3], graph = FALSE)
#PCA by symbiotic state
pca_sym_state_7.5_10 <- ggord(ord_ammonium_7.5_10, ammonium_control_7.5_10$sym_state)
pca_sym_state_7.5_10

#PCA by dose
ammonium_all_data$dose<-as.character(ammonium_control_7.5_10$dose)
pca_dose_7.5_10 <- ggord(ord_ammonium_7.5_10, ammonium_control_7.5_10$dose)
pca_dose_7.5_10
```
...
```{r PCA on ALL DATA}
# We are going to use the factominer package
#In order to visualize elipses we want everything to be a factor so lets just make everything a factor? lol
ammonium_all_data$dose <- as.factor(ammonium_all_data$dose)
ammonium_all_data$temp <- as.factor(ammonium_all_data$temp)
ammonium_all_data$sym_state <- as.factor(ammonium_all_data$sym_state)
ammonium_all_data$treat_ID <- as.factor(ammonium_all_data$treat_ID)

#The first step is to perform a PCA on all of the "active variables", aka the output variables you want to explain the data by
ammonium.pca_all <- PCA(ammonium_all_data[,1:3], scale.unit=TRUE, ncp=5, graph=T)
#ammonium_all_data: the data set used
...
#Next up we want to look at supplementary variables, that have no influence on the principal components of the analysis but help us to interpret the dimensions of variability'
ammonium.pca_all <- PCA(ammonium_all_data, scale.unit=TRUE, ncp=5, quanti.sup=c(8:9), quali.sup=c(4:7), graph=T)
#quanti.sup: vector of the indexes of the quantitative supplementary variables
#quali.sup: vector of the indexes of the qualitative supplementary variables -- we put everything into qualitative so its easier to work with

# We can also color the individuals according to the categories' centers of gravity:
plot.PCA(ammonium.pca, axes=c(1, 2), choix="ind", habillage=6, label="none")
...
#now let's plot elipses, this will only work for categorical variables
plotellipses(ammonium.pca_all, label="none")

#Now let's check out some PCA stats
dimdesc(ammonium.pca, axes=c(1,2))

```


```{r PCA on just SYMBIONT METRICS}

#The first step is to perform a PCA on all of the "active variables", aka the output variables you want to explain the data by
ammonium.pca_symbiont <- PCA(ammonium_all_data[,2:3], scale.unit=TRUE, ncp=5, graph=T)
#to do this just for PAM and R value we need to get rid of the resp_rate column and e coli of course
ammonium_sym_data <- subset(ammonium_all_data, select = -c(resp_rate, e_coli_plate) )
#In order to visualize elipses we want everything to be a factor so lets just make everything a factor? lol
ammonium_sym_data$dose <- as.factor(ammonium_sym_data$dose)
ammonium_sym_data$temp <- as.factor(ammonium_sym_data$temp)
ammonium_sym_data$sym_state <- as.factor(ammonium_sym_data$sym_state)
ammonium_sym_data$treat_ID <- as.factor(ammonium_sym_data$treat_ID)


ammonium_pca_symbiont <- PCA(ammonium_sym_data, scale.unit=TRUE, ncp=5, quanti.sup=c(7), quali.sup=c(3:6), graph=T)
#here our quanitative is dose(4), temp (5) e. coli (7) and colony number (9). Our qualitative are treat ID (3) and sym state (7)
#This makes sense, as red delta increases (bleaching) the change in PAM decreases (they lost photosynthetic efficiency)

# We can also color the individuals according to the categories' centers of gravity:
plot.PCA(ammonium_pca_symbiont, axes=c(1, 2), choix="ind", habillage=4, label="none")
#choix: the graph to plot ("ind" for the individuals, "var" for the variables)
#habillage: to choose the colours of the individuals: no colour ("none"), a colour for each individual ("ind") or to colour the individuals according to a categorical variable (give the number of the qualitative variable)

#now let's plot elipses
plotellipses(ammonium_pca_symbiont, label="none")
#doesnt come out quite as clean as I would like

#Now let's check out some PCA stats
dimdesc(ammonium_pca_symbiont, axes=c(1,2))

```


#Hannah's PCA code in case we want to check it out
