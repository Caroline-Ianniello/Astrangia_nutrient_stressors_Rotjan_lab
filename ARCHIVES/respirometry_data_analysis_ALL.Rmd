...
fullModel = lm(resp_rate ~   N_species_dose + temp + sym_state + e_coli_plate + N_species_dose*temp + N_species_dose*sym_state + N_species_dose*e_coli_plate  + temp*sym_state + temp*e_coli_plate + sym_state*e_coli_plate, data= resp_all.data) # model with all 9 variables and possible combinations

nullModel = lm(resp_rate ~  1, data= resp_all.data) # model with the intercept only
...
resp_all.data$N_species_dose <- factor(resp_all.data$N_species_dose, levels=c("CNTRL_0", "A_5", "A_7.5", "A_10", "N_1.5", "N_5","N_18"))

lm_test<- lm(formula = resp_rate ~ N_species_dose + temp + e_coli_plate + 
    sym_state, data = resp_all.data)
summary(lm_test)
elm_test=emmeans(lm_test, specs=pairwise ~ e_coli_plate*temp)
summary(elm_test)
