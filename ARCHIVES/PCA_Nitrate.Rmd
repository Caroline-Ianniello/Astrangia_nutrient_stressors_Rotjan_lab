...
nitrate_all_data <- subset(nitrate_all_data, select = -c(beetag_exprun) )


#lets make a new dataset that omits NAs because if we don't have all the data then the PCA function freaks
nitrate_all_data <-  na.omit(nitrate_all_data)
```


```{r Pretty PCAs of all independent variables}
# Using ggord package to make PCAs
ord_nitrate <- PCA(nitrate_all_data[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state <- ggord(ord_nitrate, nitrate_all_data$sym_state)
pca_sym_state

#PCA by dose
nitrate_all_data$dose<-as.character(nitrate_all_data$dose)
pca_dose <- ggord(ord_nitrate, nitrate_all_data$dose)
pca_dose

#PCA by temperature
nitrate_all_data$temp<-as.character(nitrate_all_data$temp)
pca_temp <- ggord(ord_nitrate, nitrate_all_data$temp)
pca_temp

#PCA by treatment ID
pca_treat_ID <- ggord(ord_nitrate, nitrate_all_data$treat_ID)
pca_treat_ID
...
nitrate_all_ecoli <- subset(nitrate_all_data, nitrate_all_data$e_coli_plate==1)

# Using ggord package to make PCAs
ord_nitrate <- PCA(nitrate_all_ecoli[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_ecoli <- ggord(ord_nitrate, nitrate_all_ecoli$sym_state)
pca_sym_state_ecoli 

#PCA by dose
nitrate_all_ecoli$dose<-as.character(nitrate_all_ecoli$dose)
pca_dose_ecoli  <- ggord(ord_nitrate, nitrate_all_ecoli$dose)
pca_dose_ecoli 

#PCA by temperature
nitrate_all_ecoli$temp<-as.character(nitrate_all_ecoli$temp)
pca_temp_ecoli  <- ggord(ord_nitrate, nitrate_all_ecoli$temp)
pca_temp_ecoli 

#PCA by treatment ID
pca_treat_ID_ecoli  <- ggord(ord_nitrate, nitrate_all_ecoli$treat_ID)
pca_treat_ID_ecoli 
...
```{r}
#subset data that was treated with E. coli
nitrate_all_no_ecoli <- subset(nitrate_all_data, nitrate_all_data$e_coli_plate==0)

# Using ggord package to make PCAs
ord_nitrate <- PCA(nitrate_all_no_ecoli[, 1:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_no_ecoli <- ggord(ord_nitrate, nitrate_all_no_ecoli$sym_state)
pca_sym_state_no_ecoli 

#PCA by dose
nitrate_all_no_ecoli$dose<-as.character(nitrate_all_no_ecoli$dose)
pca_dose_no_ecoli  <- ggord(ord_nitrate, nitrate_all_no_ecoli$dose)
pca_dose_no_ecoli 

#PCA by temperature
nitrate_all_no_ecoli$temp<-as.character(nitrate_all_no_ecoli$temp)
pca_temp_no_ecoli  <- ggord(ord_nitrate, nitrate_all_no_ecoli$temp)
pca_temp_no_ecoli 

#PCA by treatment ID
pca_treat_ID_no_ecoli  <- ggord(ord_nitrate, nitrate_all_no_ecoli$treat_ID)
...
# Using ggord package to make PCAs
ord_nitrate_symbiont <- PCA(nitrate_all_data[, 2:3], graph = FALSE)

#PCA by symbiotic state
pca_sym_state_symbiont <- ggord(ord_nitrate_symbiont, nitrate_all_data$sym_state)
pca_sym_state_symbiont

#PCA by dose
nitrate_all_data$dose<-as.character(nitrate_all_data$dose)
pca_dose_symbiont <- ggord(ord_nitrate_symbiont, nitrate_all_data$dose)
pca_dose_symbiont

#PCA by temperature
nitrate_all_data$temp<-as.character(nitrate_all_data$temp)
pca_temp_symbiont <- ggord(ord_nitrate_symbiont, nitrate_all_data$temp)
pca_temp_symbiont

#PCA by treatment ID
pca_treat_ID_symbiont <- ggord(ord_nitrate_symbiont, nitrate_all_data$treat_ID)
...
nitrate_all_data$dose <- as.factor(nitrate_all_data$dose)
nitrate_all_data$temp <- as.factor(nitrate_all_data$temp)
nitrate_all_data$sym_state <- as.factor(nitrate_all_data$sym_state)
nitrate_all_data$treat_ID <- as.factor(nitrate_all_data$treat_ID)
...
nitrate.pca_all <- PCA(nitrate_all_data[,1:3], scale.unit=TRUE, ncp=5, graph=T)
#nitrate_all_data: the data set used
...
nitrate.pca_all <- PCA(nitrate_all_data, scale.unit=TRUE, ncp=5, quanti.sup=c(8:9), quali.sup=c(4:7), graph=T)
...
plot.PCA(nitrate.pca, axes=c(1, 2), choix="ind", habillage=6, label="none")
...
plotellipses(nitrate.pca_all, label="none")

#Now let's check out some PCA stats
dimdesc(nitrate.pca, axes=c(1,2))
...
#The first step is to perform a PCA on all of the "active variables", aka the output variables you want to explain the data by
nitrate.pca_symbiont <- PCA(nitrate_all_data[,2:3], scale.unit=TRUE, ncp=5, graph=T)
#to do this just for PAM and R value we need to get rid of the resp_rate column and e coli of course
nitrate_sym_data <- subset(nitrate_all_data, select = -c(resp_rate, e_coli_plate) )
#In order to visualize elipses we want everything to be a factor so lets just make everything a factor? lol
nitrate_sym_data$dose <- as.factor(nitrate_sym_data$dose)
nitrate_sym_data$temp <- as.factor(nitrate_sym_data$temp)
nitrate_sym_data$sym_state <- as.factor(nitrate_sym_data$sym_state)
nitrate_sym_data$treat_ID <- as.factor(nitrate_sym_data$treat_ID)


nitrate_pca_symbiont <- PCA(nitrate_sym_data, scale.unit=TRUE, ncp=5, quanti.sup=c(7), quali.sup=c(3:6), graph=T)
...
# We can also color the individuals according to the categories' centers of gravity:
plot.PCA(nitrate_pca_symbiont, axes=c(1, 2), choix="ind", habillage=4, label="none")
#choix: the graph to plot ("ind" for the individuals, "var" for the variables)
#habillage: to choose the colours of the individuals: no colour ("none"), a colour for each individual ("ind") or to colour the individuals according to a categorical variable (give the number of the qualitative variable)

#now let's plot elipses
plotellipses(nitrate_pca_symbiont, label="none")
#doesnt come out quite as clean as I would like

#Now let's check out some PCA stats
dimdesc(nitrate_pca_symbiont, axes=c(1,2))
