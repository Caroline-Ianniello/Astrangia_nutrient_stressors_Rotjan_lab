...
ammonium_SICB_model <- lmer(LOG_abs_resp_rate_SA_corr ~ N_species_dose + temp + red_day11 + e_coli_plate + N_species_dose*temp + N_species_dose*e_coli_plate + e_coli_plate*temp + e_coli_plate*red_day11 + N_species_dose*red_day11 + temp*red_day11 + N_species_dose*red_day11*temp + N_species_dose*temp*e_coli_plate + N_species_dose*red_day11*e_coli_plate + temp*red_day11*e_coli_plate + N_species_dose*red_day11*temp*e_coli_plate + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_ammonium)
...
(summary(emmeans(ammonium_SICB_model, specs=pairwise ~ temp*e_coli_plate)))
#TEMPERATURE IS SIGNFICIANT, pairwise, with OR without E. coli


#just keep everything fed NO E COLI
SICB_data_ammonium_noEC <- resp_all_ammonium.data %>%
  filter(food=="F") %>%
  filter(e_coli_plate==0) %>%
 filter(abs_resp_rate_SA_corr<0.3) #BECAUSE THERES AN OUTLIER around 0.3 for temp 30! KEEP TRACK OF ME
#create an lmer with this dataset
ammonium_SICB_model_noEC <- lmer(LOG_abs_resp_rate_SA_corr ~ N_species_dose + temp + red_day11 +  N_species_dose*temp + N_species_dose*red_day11 + temp*red_day11 + N_species_dose*red_day11*temp  + N_species_dose*red_day11*temp+ (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_ammonium_noEC)
# boundary (singular) fit: see help('isSingular') HAPPENS WHEN RANDOM EFFECTS ARE VERY SMALL
...
ammonium_SICB_model_RGB_noEC <- lmer(red_day11 ~ N_species_dose + temp + N_species_dose*temp  + N_species_dose*temp  + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_ammonium_noEC)
summary(ammonium_SICB_model_RGB_noEC)
(summary(emmeans(ammonium_SICB_model_RGB, specs=pairwise ~ temp))) 

library(ggplot2)
ggplot(data = SICB_data_ammonium,
       aes(x = red_day1,
           y = sym_state, color=temp)) +
    geom_point() +
  geom_abline()
