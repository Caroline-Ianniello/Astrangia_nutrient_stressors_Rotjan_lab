...
resp_all_nitrate.data <- read.csv("RESP_Nitrate_Cleaned.csv", header=TRUE, sep=",",stringsAsFactors = FALSE) #I added a new column called "control_or_nutrient" to make some plotting easier
...
#Linear models - we are NOT going to include genet as a random factor here because we don't care too much about the genet

```{r linear models - ALL NITRATE DATA}

#stepAIC is from the MASS package
fullModel = lm(resp_rate ~   N_species_dose + temp + sym_state + e_coli_plate + N_species_dose*temp + N_species_dose*sym_state + N_species_dose*e_coli_plate  + temp*sym_state + temp*e_coli_plate + sym_state*e_coli_plate, data= resp_all_nitrate.data) # model with all 9 variables and possible combinations
...
mm_resp_null <- lm(resp_rate ~ 1 + (1|vial_ID) + (1|col_num) + (1|exp_num) +(1|exp_subrun) +(1|jar_unique_ID) + (1|resp_run) , data= resp_all_nitrate.data)
summary(mm_resp_null)
#that null looks good, intercept is significant

resp_random_model <- lm(resp_rate ~ N_species_dose + food +(1|vial_ID), data=resp_all_nitrate.data)
summary(resp_random_model)



#full model with all interactions possible
mm_resp_1 <- lm(resp_rate ~  N_species_dose + temp + food + sym_state + e_coli_plate + N_species_dose*temp + N_species_dose*food + N_species_dose*sym_state + N_species_dose*e_coli_plate + temp*food + temp*sym_state + temp*e_coli_plate + food*sym_state + food*e_coli_plate + sym_state*e_coli_plate + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= resp_all_nitrate.data)
summary(mm_resp_1)
#WARNING:
...
full.model <- lm(resp_rate ~  N_species_dose + temp + food + sym_state + e_coli_plate + N_species_dose*temp + N_species_dose*food + N_species_dose*sym_state + N_species_dose*e_coli_plate + temp*food + temp*sym_state + temp*e_coli_plate + food*sym_state + food*e_coli_plate + sym_state*e_coli_plate +  + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= resp_all_nitrate.data)
...
mm_resp_2 <- lm(resp_rate ~ N_species_dose + temp + e_coli_plate + N_species_dose*e_coli_plate, data=resp_all_nitrate.data)
summary(mm_resp_2)
...
mm_resp_4 <- lm(resp_rate ~ N_species_dose + temp + e_coli_plate + sym_state+ (1 | col_num) + N_species_dose*e_coli_plate, data=resp_all_nitrate.data)
summary(mm_resp_4)

AIC(mm_resp_2, mm_resp_4)

# lets add sym state for fun and see what happens
mm_resp_3 <- lm(resp_rate ~ N_species_dose + temp + e_coli_plate + sym_state + (1 | col_num), data=resp_all_nitrate.data)
...
mm_resp_control_or_nutrient <- lm(resp_rate ~ control_or_nutrient + temp + e_coli_plate + (1 | col_num) + control_or_nutrient*e_coli_plate, data=resp_all_nitrate.data)
summary(mm_resp_control_or_nutrient)

emm_test=emmeans(mm_resp_control_or_nutrient, specs=pairwise ~ control_or_nutrient*e_coli_plate)
summary(emm_test)

tab_model(mm_resp_control_or_nutrient)
```
...
mm_resp_2 <- lm(resp_rate ~ N_species_dose + temp + e_coli_plate + (1 | col_num) + N_species_dose*e_coli_plate, data=resp_all_nitrate.data)
summary(mm_resp_2)
...
gg_N_species_dose_e_coli_nitrate<-ggplot(data = sum_N_species_dose_e_coli_nitrate,
   ...
   # scale_x_discrete(labels=c("20C","30C"))+
    labs(x = "Nitrate dose", 
         ...
         caption = "data represent mean +/- SEM" ) +
   scale_x_discrete("Nitrate dose", labels=c(CNTRL_0="Control", N_1.5="1.5uM", N_5="5uM", N_18="18uM"))+
     scale_color_manual("E. coli presence", labels=c("1"="E. coli", "0"="No E. coli"), values=c("gray", "darkgreen"))
...
mm_without_controls <-lm(resp_rate ~ N_species_dose + sym_state + e_coli_plate + temp + (1 | col_num), data=nitrate_without_control)

summary(mm_without_controls)

# Summarize by temperature
emm_temp=emmeans(mm_without_controls, specs=pairwise ~ sym_state)
summary(emm_temp)
...
#Pairwise by 
emm_e_coli=emmeans(mm_without_controls, specs=pairwise ~ temp*e_coli_plate)
summary(emm_e_coli)
...
mm_chalktalk<- lm(resp_rate ~  N_species_dose , data= all_sym)
...
# STATS STATS STATS
mm_resp_control_v_nutrient <- lm(resp_rate ~  temp + e_coli_plate + control_or_nutrient + (1 | col_num), data=resp_all_ammonium.data)
...
emm_control_v_nutrient=emmeans(mm_resp_control_v_nutrient, specs=pairwise ~ control_or_nutrient*temp)
summary(emm_control_v_nutrient)

# Summarize by control_or_nutrient * symbiotic state
emm_control_v_nutrient=emmeans(mm_resp_control_v_nutrient, specs=pairwise ~ control_or_nutrient*sym_state)
summary(emm_control_v_nutrient)
...
mm_controls <-lm(resp_rate ~ sym_state + e_coli_plate + temp + (1 | col_num), data=nitrate_controls)
...
mm_resp_TREAT_ID <- lm(resp_rate ~ treat_ID_collapsed_controls + (1|exp_num) + (1|col_num), data= resp_all_nitrate.data)
...
mm_fed_nitrate <- lm(resp_rate ~  N_species_dose + temp + sym_state + e_coli_plate +  (1|col_num) , data= all_fed_nitrate)
summary(mm_fed_nitrate)
...
mm_starved_nitrate <- lm(resp_rate ~ N_species_dose + temp + sym_state + e_coli_plate +  (1|col_num) , data= all_starved_nitrate)
summary(mm_starved_nitrate)
