...
```{r E. COLI NITRATE BY SYM STATE and FOOD}
e_coli_sym_fed<-subset(e_coli, resp_all_nitrate.data$sym_state=="S" & resp_all_nitrate.data$food=="F")
e_coli_sym_starved<-subset(e_coli, resp_all_nitrate.data$sym_state=="S" & resp_all_nitrate.data$food=="S")
e_coli_apo_fed<-subset(e_coli, resp_all_nitrate.data$sym_state=="A" & resp_all_nitrate.data$food=="F")
e_coli_apo_starved<-subset(e_coli, resp_all_nitrate.data$sym_state=="A" & resp_all_nitrate.data$food=="S")

# E coli sym fed
mm_e_coli_sym_fed <- lmer(resp_rate ~  N_species_dose + temp + N_species_dose*temp + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= e_coli_sym_fed)
summary(mm_e_coli_sym_fed)
print(step.model<- step(mm_e_coli_sym_fed))
#nothing really significant

# E coli sym starved
mm_e_coli_sym_starved <- lmer(resp_rate ~  N_species_dose + temp + N_species_dose*temp + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= e_coli_sym_starved)
summary(mm_e_coli_sym_starved)
print(step.model<- step(mm_e_coli_sym_starved))
#nothing really significant

# E coli apo fed
mm_e_coli_apo_fed <- lmer(resp_rate ~  N_species_dose + temp + N_species_dose*temp + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= e_coli_apo_fed)
summary(mm_e_coli_apo_fed)
print(step.model<- step(mm_e_coli_apo_fed))
# nothing is really significant
lm_e_coli_apo_fed<-lm(resp_rate ~ N_species_dose, data=e_coli_apo_fed)
summary(emmeans(lm_e_coli_apo_fed, pairwise ~ N_species_dose)) # Nothing significant

# E coli apo starved
mm_e_coli_apo_starved <- lmer(resp_rate ~  N_species_dose + temp + N_species_dose*temp + (1|vial_ID) +  (1|col_num) + (1|exp_num) + (1|exp_subrun), data= e_coli_apo_starved)
summary(mm_e_coli_apo_starved)
print(step.model<- step(mm_e_coli_apo_starved))
# temperature is significant!
lm_e_coli_apo_starved<-lm(resp_rate ~ temp, data=e_coli_apo_starved)
summary(emmeans(lm_e_coli_apo_starved, pairwise ~ temp)) # TEMP IS SIGNIFICANT, 20C respired more than 30C
