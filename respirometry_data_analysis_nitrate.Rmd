---
title: "Respirometry data analysis (NITRATE) C. Fleming Last updated: 2/13/22"
output:
...
library(glmmTMB)
#install.packages("relaimpo") #for qauntifying the effects of predictor variables
library(relaimpo)
...
getwd()
resp_all.data <- read.csv("RESP_ALL_SUMMARY_cleaned_1_27_23.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
...
mm_resp_TREAT_ID <- lmer(resp_rate ~ treat_ID_collapsed_controls + (1|vial_ID) + (1|col_num), data= resp_all_nitrate.data)
...
mm_resp_null <- lmer(resp_rate ~ 1 + (1|vial_ID) + (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_null)
#that null looks good

#full model with all interactions possible
mm_resp_1 <- lmer(resp_rate ~ N_species_dose + temp + food + sym_state + e_coli_plate + N_species_dose*temp + N_species_dose*food + N_species_dose*sym_state + N_species_dose*e_coli_plate + temp*food + temp*sym_state + temp*e_coli_plate + food*sym_state + food*e_coli_plate + sym_state*e_coli_plate + (1|vial_ID)+ (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_1)
#WARNING:
# boundary (singular) fit: see help('isSingular')
help('isSingular')

mm_resp_2 <- lmer(resp_rate ~ N_species_dose +  temp + sym_state + e_coli_plate + (1|vial_ID)+ (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_2)
...
#  CNTRL_0 - N_1.5 -0.000251 0.000211 281.6  -1.186  0.6362
#  CNTRL_0 - N_18   0.000400 0.000215 109.9   1.860  0.2514
#  CNTRL_0 - N_5    0.000210 0.000215 176.0   0.977  0.7630
#  N_1.5 - N_18     0.000651 0.000245  59.9   2.661  0.0478 *** the only one that is statistically significant
#  N_1.5 - N_5      0.000460 0.000241 143.9   1.908  0.2295
#  N_18 - N_5      -0.000191 0.000244 117.4  -0.782  0.8625
    # Answer: There is no specific dose where this occurs overall. 
...
#Tukey post hoc says:
# contrast         estimate       SE  df t.ratio p.value
# temp20 - temp30 -0.000417 0.000157 296  -2.662  0.0082 *** statistically signficant
    # Answer: Yes! Corals at 30C respired less than those at 20C.
mean(all_20C$resp_rate, na.rm=TRUE)
#-0.004589156 #respired more than 30C
...
#Tukey post hoc says:
# contrast estimate       SE  df t.ratio p.value
# A - S    0.000403 0.000208 332   1.941  0.0531
...
# -0.004610212 #respired more than apo
...
#contrast                      estimate       SE  df t.ratio p.value
# e_coli_plate0 - e_coli_plate1 0.000429 0.000175 456   2.450  0.0147
  #Answer: E. coli plates respired more than plates without e coli.
mean(e_coli$resp_rate, na.rm=TRUE)
# -0.004565063 #respired more than no_e_coli
mean(no_e_coli$resp_rate, na.rm=TRUE)
# -0.004228692

mm_resp_3 <- lmer(resp_rate ~ N_species_dose +  temp + e_coli_plate + (1|vial_ID)+ (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_3)
emm_resp_3=emmeans(mm_resp_3, specs=pairwise ~ N_species)
summary(emm_resp_3)

AIC(mm_resp_2, mm_resp_3) #remember, lower AIC wins
#3 is better but it is marginal and we think sym state is important to the story so we will keep it in

mm_resp_test <- lmer(resp_rate ~ temp + sym_state + e_coli_plate + (1|vial_ID)+ (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_test)
emmeans(mm_resp_test, specs=pairwise ~ N_species)
summary(emm_resp_test)

summary(aov(resp_rate ~ e_coli_plate, data=resp_all_nitrate.data))

mm_resp_3_a <- lmer(resp_rate ~ N_dose  + temp + sym_state + e_coli_plate + N_dose*temp + N_dose*sym_state + sym_state*temp + (1|vial_ID), data= resp_all_nitrate.data)
summary(mm_resp_3_a)

mm_resp_4 <- lmer(resp_rate ~ N_dose  + temp + sym_state + e_coli_plate +  N_dose*sym_state + sym_state*temp + (1|vial_ID), data= resp_all_nitrate.data)
summary(mm_resp_4)

mm_resp_5 <- lmer(resp_rate ~ N_dose  + temp + sym_state + e_coli_plate + sym_state*temp + (1|vial_ID), data= resp_all_nitrate.data)
summary(mm_resp_5)

mm_resp_6 <- lmer(resp_rate ~ N_dose  + temp + sym_state + e_coli_plate +  N_dose*sym_state + (1|vial_ID), data= resp_all_nitrate.data)
summary(mm_resp_6)
...
#lower AIC wins, mm_resp_2 wins (we need to use our biological brain, because im not super interested in the interactions)
...
#This is the model it suggests (it exactly the same as linear model 2 suggested above):
lm_resp_2 <- lm(formula = resp_rate ~ sym_state + temp + N_species_dose + 
    e_coli_plate, data = resp_all_nitrate.data)
summary(lm_resp_2)
#So adding the random effects we go with our resp_mm_2
mm_resp_2 <- lmer(resp_rate ~ N_species_dose +  temp + sym_state + e_coli_plate + (1|vial_ID)+ (1|col_num), data= resp_all_nitrate.data)
summary(mm_resp_2)
#now let's do some model exploration
...
r2(mm_resp_2) #marginal id variance just by fixed factors, conditional includes variance explained by random factor
  # treatment explains 6.6% of variation in ln.sum.arms
...
plot_model(mm_resp_2, type="diag")
...
VarCorr(mm_resp_2) #seems like a really low correlation-- they are not super important
...
library(relaimpo)
lm_resp_2_relimp <- lm(resp_rate ~ factor(sym_state) + factor(temp) + factor(N_species_dose) + 
...
mm_resp_e_coli_1 <- lmer(resp_rate ~ N_species_dose + temp + food + sym_state + (1|vial_ID), data= e_coli)
summary(mm_resp_e_coli_1)
# the factors that are not significant and I will drop: food, temp

mm_resp_e_coli_2 <- lmer(resp_rate ~ N_species_dose  + sym_state + (1|vial_ID) + (1|col_num), data= e_coli)
summary(mm_resp_e_coli_2)

AIC(mm_resp_e_coli_1, mm_resp_e_coli_2) #1 wins the AIC weirdly
