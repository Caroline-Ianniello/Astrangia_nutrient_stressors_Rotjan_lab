...
(summary(emmeans(nitrate_SICB_model, specs=pairwise ~ temp*e_coli_plate)))
...
# THE STORY HERE
#What we DONT see: significant effect of dose or temperature or symbiotic state
#What we DO see: a significant effect of E. coli, increase in respiration rate
#Interestingly, though, we see no significant difference in resp rates at 20 v 30C with NO e coli, but we do see significant metabolic depression at 30C when they are exposed to E. coli
# When we give them an immune challenge, we see that 20 vs. 30 -- 30 has depression
# The immune challenge is the straw that broke the camel's back -- they do fine without it, but with it they tend to exhibit depression, sublethal effect not being able to mount a proper response (KEY POINT, I think metabolic depression is bad here, is that safe to say?)
...
SICB_data_nitrate_noEC <- resp_all_nitrate.data %>%
 ...
 filter(abs_resp_rate_SA_corr<0.7) %>% #there is one 7.5 at 20C thats really high and looks like an outlier to me
  filter(beetag_exprun!= "G33Y97_exp2A") #this was at E coli at 30C, and it was at like 0.23 and also looked like an outlier
nitrate_SICB_model_noEC <- lmer(LOG_abs_resp_rate_SA_corr ~ N_species_dose + temp + red_day11 +  N_species_dose*temp + N_species_dose*red_day11 + temp*red_day11 + N_species_dose*red_day11*temp  + N_species_dose*red_day11*temp+ (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate_noEC)
...
nitrate_SICB_model_RGB <- lmer(red_day11 ~ N_species_dose + temp + sym_state + N_species_dose*temp + N_species_dose*sym_state + temp*sym_state + N_species_dose*sym_state*temp  + (1 | col_num)  + (1 | jar_unique_ID), data= SICB_data_nitrate)
summary(nitrate_SICB_model_RGB)
(summary(emmeans(nitrate_SICB_model_RGB, specs=pairwise ~ temp))) 
