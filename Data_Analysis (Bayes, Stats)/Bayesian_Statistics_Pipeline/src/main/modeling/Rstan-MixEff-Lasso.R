...
rstan_mixEff_lasso <- function(data, formula, location = 0, lambda, chains = 4, iter = 2000, refresh = 0, adapt_delta = NULL, QR = FALSE, sparse = FALSE, autoscale = TRUE){
  ...
  fit <- stan_glmer(formula, data = data, family = gaussian(), 
                    prior = laplace(location = location, scale = lambda, autoscale = autoscale), 
...
customizedRandomizedSearchCV <- function(data, formula, fold_number, stratified_target, MCMC_parms = NULL, autoscale = TRUE, randomseed = NULL, lambda_dist, ...){
  ...
  # randomseed: <int> random state to control the samples of parameters of Laplace prior
  # autoscale: <bool> A logical scalar (defaulting to TRUE) indicating whether the scale of the Laplace should be auto adjusted.
  ...
  # sample lambda candidates from given distribution
...
customizedGridSearchCV <- function(data, formula, fold_number, stratified_target, MCMC_parms = NULL, autoscale = TRUE, params_value_vec){
  ...
  # autoscale: <bool> A logical scalar (defaulting to TRUE) indicating whether the scale of the Laplace should be auto adjusted.
  # params_value_vec: Since we are only tuning the parameter "lambda" of laplace distribution, simplifying the program by giving a vector.
  # =====
  # Return: <list> MSE_array
  # illustration of matrix for first two dimensions
  # =====
