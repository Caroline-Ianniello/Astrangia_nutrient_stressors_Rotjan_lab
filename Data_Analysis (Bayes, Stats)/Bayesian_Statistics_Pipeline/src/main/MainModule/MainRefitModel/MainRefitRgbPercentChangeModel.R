...
rgb_model$temp <- factor(rgb_model$temp, levels = c(20, 29))
...
print(rgbperc_best_lambda) # 0.9954275
...
RGB_percChange_lasso <- refitRstanLasso(data = rgb_model,
                                        formula = as.formula('red_percent_change ~ (dose_level + temp + feed + symbiont)^4 + (1|col_num)'),
                                        best_lambda = rgbperc_best_lambda,
                                        iter = 4000,
                                        autoscale = TRUE)
...
save(RGB_percChange_lasso, file = 'MSSP/data/Refit-Models/RGB-Percent-Change/RGB_percChange_lasso.RDS')
