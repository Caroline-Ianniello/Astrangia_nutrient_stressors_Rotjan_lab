...
rgbdelta_best_lambda <- find_best_tune(rgb_delta_MSE_arr)

print(rgbdelta_best_lambda) # 1.007505
...
RGB_delta_lasso <- refitRstanLasso(data = rgb_model,
                                   formula = as.formula('red_delta ~ (dose_level + temp + feed + symbiont)^4 + (1|col_num)'),
                                   best_lambda = rgbdelta_best_lambda,
                                   iter = 4000, 
                                   autoscale = TRUE)
...
save(RGB_delta_lasso, file = 'MSSP/data/Refit-Models/RGB-Delta/RGB_delta_lasso.RDS')
