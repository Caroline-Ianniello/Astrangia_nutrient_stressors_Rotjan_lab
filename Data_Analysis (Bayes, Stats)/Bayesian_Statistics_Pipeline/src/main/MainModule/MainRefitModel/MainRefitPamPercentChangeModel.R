...
pamperc_best_lambda <- find_best_tune(pam_percchange_MSE_arr)

print(pamperc_best_lambda) # 1.007505
...
PAM_percChange_lasso <- refitRstanLasso(data = pam_model,
                                        formula = as.formula('pam_percent_change ~ (dose_level + temp + feed + symbiont)^4 + (1|col_num)'),
                                        best_lambda = pamperc_best_lambda,
                                        iter = 4000, 
                                        autoscale = TRUE)
...
save(PAM_percChange_lasso, file = 'MSSP/data/Refit-Models/PAM-Percent-Change/PAM_percChange_lasso.RDS')
