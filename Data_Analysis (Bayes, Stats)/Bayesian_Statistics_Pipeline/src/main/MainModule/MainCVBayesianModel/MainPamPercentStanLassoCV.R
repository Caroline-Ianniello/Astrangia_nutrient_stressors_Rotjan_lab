...
pam_model$dose_level <- relevel(factor(pam_model$dose_level), ref = 'Low') # make sure using Low as reference


# Customized CV -----------------------------------------------------------

set.MC.cores(cores = 4)
pam_percchange_MSE_arr <- customizedRandomizedSearchCV(data = pam_model, 
...
save(pam_percchange_MSE_arr, file = 'MSSP/data/Cross-Validation-Results/MSE-Arrays/pam_percchange_MSE_arr.RDS')
