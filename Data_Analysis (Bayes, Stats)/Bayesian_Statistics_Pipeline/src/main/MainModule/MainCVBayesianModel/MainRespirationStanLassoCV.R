...
resp_model$dose_level <- relevel(factor(resp_model$dose_level), ref = 'Low') # make sure using Low as reference
...
resp_MSE_arr <- customizedRandomizedSearchCV(data = resp_model,
...
save(resp_MSE_arr, file = 'MSSP/data/Cross-Validation-Results/MSE-Arrays/resp_MSE_arr.RDS')
