...
PAM_day1_11_summary$day_NO3 <- paste(PAM_day1_11_summary$NO3_category, PAM_day1_11_summary$day, sep="_")

PAM_day1_11_summary_BACKGROUND <- subset(PAM_day1_11_summary, PAM_day1_11_summary$NO3_category=="Low")

PAM_day1_11_summary_ELEVATED <- subset(PAM_day1_11_summary, PAM_day1_11_summary$NO3_category=="Elevated")

#FOOOOD
sum_day1_temp_food <- summarySE(PAM_all.data, measurevar = c("PAM_avg_day1"), groupvars = c("food", "temp", "NO3_category"))
sum_day1_temp_food$day <- 1
sum_day1_temp_food$PAM_avg <- sum_day1_temp_food$PAM_avg_day1

sum_day11_temp_food <- summarySE(PAM_all.data, measurevar = c("PAM_avg_day11"), groupvars = c("food", "temp", "NO3_category"))
sum_day11_temp_food$day <- 11
sum_day11_temp_food$PAM_avg <- sum_day11_temp_food$PAM_avg_day11


PAM_day1_11_summary_food <- full_join(sum_day1_temp_food, sum_day11_temp_food)
PAM_day1_11_summary_food$day <- as.factor(PAM_day1_11_summary_food$day)
PAM_day1_11_summary_food$day_NO3 <- paste(PAM_day1_11_summary_food$NO3_category, PAM_day1_11_summary_food$day, sep="_")

PAM_day1_11_summary_BACKGROUND <- subset(PAM_day1_11_summary_food, PAM_day1_11_summary_food$NO3_category=="Low")

PAM_day1_11_summary_ELEVATED <- subset(PAM_day1_11_summary_food, PAM_day1_11_summary_food$NO3_category=="Elevated")


# COMBINED PAM DELTA PLOT
# Create the shaded areas data

# PAM Symbiotic State Plot
gg_sym_state_PAM <- ggplot(data = PAM_day1_11_summary,
                           aes(x = day_NO3,
                               y = PAM_avg, col = temp, shape = sym_state, group = interaction(sym_state, temp, NO3_category))) +
  geom_point(size = 4.5, position = position_dodge(width = 0.45)) +
  geom_errorbar(aes(ymin = PAM_avg - se, ymax = PAM_avg + se), width = 0.455, linewidth = 0.8, position = position_dodge(width = 0.45)) +
  geom_line(linewidth = 1, position = position_dodge(width = 0.45), linetype = "dotted") +
  #ggtitle("PAM N Delta By Temp Sym State") +
  theme_classic() +
...
#PAM Food plot
gg_food_PAM <- ggplot(data = PAM_day1_11_summary_food,
                           aes(x = day_NO3,
                               y = PAM_avg, col = temp, shape = food, group = interaction(food, temp, NO3_category))) +
  geom_point(size = 4.5, position = position_dodge(width = 0.45)) +
  geom_errorbar(aes(ymin = PAM_avg - se, ymax = PAM_avg + se), width = 0.455, linewidth = 0.8, position = position_dodge(width = 0.45)) +
  geom_line(linewidth = 1, position = position_dodge(width = 0.45), linetype = "dotted") +
  # Add shaded rectangles for highlighting
 # ggtitle("PAM N Delta By Temp Sym State") +
  theme_classic() +
...
# Combine the two plots, sharing the x-axis and legend
gg_PAM_DELTA <- gg_sym_state_PAM / gg_food_PAM +  # Stack the plots vertically
...
ggsave(file="Figures/gg_PAM_DELTA.svg", plot=gg_PAM_DELTA, device='svg')
ggsave(file="Figures/gg_PAM_DELTA.png", plot=gg_PAM_DELTA)
...
```{r Adapting after Randis edits Sept 15}

gg_sym_state_PAM <- ggplot(data = PAM_day1_11_summary,
                           aes(x = day_NO3,
                               y = PAM_avg, col = temp, shape = sym_state, group = interaction(sym_state, temp, NO3_category))) +  # Group by sym_state, temp, and NO3_category
  geom_point(size = 4.5, position = position_dodge(width = 0.45)) +
  geom_errorbar(aes(ymin = PAM_avg - se, ymax = PAM_avg + se), width = 0.455, linewidth = 0.8, position = position_dodge(width = 0.45)) +  # Keep solid error bars
...
gg_food_PAM <- ggplot(data = PAM_day1_11_summary_food,
                      aes(x = day_NO3,
                          y = PAM_avg, col = temp, shape = food, group = interaction(food, temp, NO3_category))) +  # Group by food, temp, and NO3_category
  geom_point(size = 4.5, position = position_dodge(width = 0.45)) +
  geom_errorbar(aes(ymin = PAM_avg - se, ymax = PAM_avg + se), width = 0.455, linewidth = 0.8, position = position_dodge(width = 0.45)) +  # Keep solid error bars
  geom_line(aes(linetype = "Dotted line"), linewidth = 1, position = position_dodge(width = 0.45)) +  # Dotted lines and include in the legend
  geom_vline(xintercept = 2.5, color = "black", linetype = "solid", size = 1) +  # Black line separating NO3 categories
...
gg_PAM_DELTA <- gg_sym_state_PAM / plot_spacer() / gg_food_PAM +  # Add plot_spacer() between the plots
...
gg_sym_temp <-ggplot(data = sum_sym_temp,
    ...
    ylim(-0.18,0.15) +
  scale_x_discrete("Treatment", labels=c("Elevated","Background"))+
    labs(x = "Treatment", 
