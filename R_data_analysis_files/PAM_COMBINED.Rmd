...
PAM_all.data<- read.csv("C:/Users/carol/OneDrive/Documents/PhD Boston University 2019-/Astrangia_nutrient_stressors_Rotjan_lab/Data/PAM_dead_deleted_combined_NO3categorized_6_26_24.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
...
hist(PAM_all.data$PAM_delta) #nice and normal

#Now an ANOVA on sym state, temp, and NO3_category 
anova_PAM <- aov(PAM_delta ~ sym_state*temp*NO3_category, data=PAM_all.data)
summary(anova_PAM )
#ptemp = 0.0146
TukeyHSD(anova_PAM )
...
#For Manuscript
```{r PAM Delta visuals}
sum_sym_temp_nitrate_PAM <- summarySE(PAM_all.data, measurevar = "PAM_delta", groupvars = c("sym_state", "temp", "NO3_category"))
# Visualize
gg_sum_sym_temp_nitrate_PAM <-ggplot(data = sum_sym_temp_nitrate_PAM ,
       aes(x = NO3_category,
           y = PAM_delta, col=temp, shape=sym_state)) +
    geom_point(size=4) +
    geom_errorbar(aes(ymin=PAM_delta-se, ymax=PAM_delta+se), width=0.25, linewidth=0.8) +
    ggtitle ("Delta Photosynthetic Efficiency Value by Sym State and Temp") +
...
gg_sum_sym_temp_nitrate_PAM 

```


```{r PAM Day 11 visuals}
sum_sym_temp_nitrate_PAM <- summarySE(PAM_all.data, measurevar = "PAM_avg_day11", groupvars = c("sym_state", "temp", "NO3_category"))
# Visualize
gg_sum_sym_temp_nitrate_PAM <-ggplot(data = sum_sym_temp_nitrate_PAM ,
       aes(x = NO3_category,
