...
rgb_all_nitrate.data <- read.csv("C:/Users/carol/OneDrive/Documents/PhD Boston University 2019-/Astrangia_nutrient_stressors_Rotjan_lab/Data/RGB_NITRATE_Recomputted_dead_deleted_5_16_24.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
...
#now visualize
gg_outliers_check<-
  ggplot(rgb_all_nitrate.data, aes(x=N_species_dose, y=red_delta, col=sym_state)) +
...
scale_color_manual("Sym State",labels=c("Apo","Sym"), values=c("darkgray", "brown"))+
  ggtitle("Red delta by dose - nitrate")
gg_outliers_check

gg_outliers_check<-
  ggplot(rgb_all_nitrate.data, aes(x=N_species_dose, y=red_percent_change, col=sym_state)) +
  geom_boxplot(width=0.8, outlier.shape = NA) +
  geom_jitter(width=0.15, alpha=0.9) + #this overlays the raw data 
 ...
 annotate("text", x = Inf, y = Inf, label = paste("  "), vjust = 1, hjust = 1, size=3)+
      scale_color_manual("Sym State",labels=c("Apo","Sym"), values=c("darkgray", "brown"))+
  ggtitle("Red percent change - nitrate")
gg_outliers_check
...
# all 20C FED
all_20_fed<-subset(rgb_all_nitrate.data, rgb_all_nitrate.data$temp=="20" & rgb_all_nitrate.data$food=="F")
# all 30C FED
all_30_fed<-subset(rgb_all_nitrate.data, rgb_all_nitrate.data$temp=="30" & rgb_all_nitrate.data$food=="F")
...
all_20_fed$temp<- as.factor(all_20_fed$temp)
...
#Significant bleaching at 18uM of nitrate, significant bleaching
#significant bleaching at 30C
...
gg_sym_state_temp_nitrate<-ggplot(data = all_fed_model_vis,
    ...
    ggtitle ("nitrate rgb - % Change Fv/Fm") +
    labs(x = "nitrate dose", 
...
lm_rgb_2 <- lm(formula = red_delta ~ sym_state*N_species_dose + N_species_dose*temp, data = rgb_all_nitrate.data)
summary(lm_rgb_2)
#nothing going on
lm_rgb_3 <- lm(formula = red_delta ~ sym_state + N_species_dose, data = rgb_all_nitrate.data)
summary(lm_rgb_3)
...
summary(aov_temp<-aov(red_delta ~ control_or_nutrient, data= rgb_all_nitrate.data))
...
# I guess well go with 2 but its not great
...
summary(lm_rgb_3)
emm_apo_N_species_dose=emmeans(lm_rgb_3, specs=pairwise ~ sym_state)
summary(emm_apo_N_species_dose)
#lets go with this cause its next best AIC after 1
...
summary(aov_temp<-aov(red_percent_change ~ control_or_nutrient, data= rgb_all_nitrate.data))
Anova(aov_temp, type="III")
...
summary(lm_sym_2)
#WINNER, but still nothing really going on


AIC(lm_sym_1, lm_sym_2)

emm_sym_rgb_N_species_dose_temp=emmeans(lm_sym_2, specs=pairwise ~ temp)
summary(emm_sym_rgb_N_species_dose_temp)
