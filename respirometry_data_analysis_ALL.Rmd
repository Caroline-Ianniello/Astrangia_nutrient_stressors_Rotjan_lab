---
title:  "Respirometry data analysis (ALL) C. Fleming Last updated: 2/13/22"
output:
...
#import the data once ive taken all those out, I want to see if E coli is still significant / explains variation
resp_all.data_TEST_1 <- read.csv("RESP_ALL_SUMMARY_cleaned_1_27_23_testing_something_ignore_me.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
#now lets check if those treatments are driving the linear model
mm_resp_test <- lmer(resp_rate ~ N_species_dose +  temp + sym_state + e_coli_plate + (1|vial_ID), data= resp_all.data_TEST_1)
summary(mm_resp_test)
#E coli does still explain variation, p value of 0.02 in the overall model
