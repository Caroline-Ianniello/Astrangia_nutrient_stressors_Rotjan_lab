...
#DELTA models (no transformation)
...
lm_pam_4 <- lm(formula = PAM_delta ~  N_species_dose + sym_state + temp, data = pam_all_nitrate.data)
summary(lm_pam_4)
#nothing going on
lm_pam_5 <- lm(formula = PAM_delta ~N_species_dose, data = pam_all_nitrate.data)
summary(lm_pam_5)

AIC(lm_pam_1, lm_pam_2, lm_pam_4, lm_pam_5)
...
r2(lm_pam_3)
#control or nutrient
summary(aov_temp<-aov(PAM_delta~ control_or_nutrient, data=pam_all_nitrate.data))
...
lm_sym_3 <- lm(PAM_delta ~  N_species_dose, data= all_sym)
summary(lm_sym_3)
#WINNER!

lm_sym_4 <- lm(PAM_delta ~  temp, data= all_sym)
summary(lm_sym_4)

AIC(lm_sym_1, lm_sym_3)

#there is not that huge of an AIC difference between 1 and 3 so lets go with 3

emm_sym_pam_N_species_dose =emmeans(lm_sym_3, specs=pairwise ~ N_species_dose)
summary(emm_sym_pam_N_species_dose)
...
gg_N_species_dose_temp_nitrate<-ggplot(data = sum_N_species_dose_temp_nitrate,
    ...
    ggtitle ("SYM CORALS nitrate rgb - Delta Fv/Fm") +
    labs(x = "nitrate dose", 
...
Anova(aov_temp, type="III")
summary(emmeans(aov_temp, specs=pairwise ~ control_or_nutrient))
...
gg_N_species_dose_temp_nitrate<-ggplot(data = sum_N_species_dose_temp_nitrate,
    ...
    geom_errorbar(aes(ymin=PAM_delta-se, ymax=PAM_delta+se), width=0.1, size = 1) +
    #scale_color_manual(values = c("black", "darkgreen")) +
    ggtitle ("APO CORALS nitrate rgb - Delta Fv/Fm") +
    labs(x = "nitrate dose", 
         y = expression(paste("Delta Fv/Fm")), 
         caption = "data represent mean +/- SEM" ) +
    scale_x_discrete("nitrate dose", labels=c(CNTRL_0="Control", N_1.5="1.5uM", N_5="5uM", N_18="18uM"))+
...
#stepAIC is from the MASS package
fullModel = lm(log_pam_percent_change ~   N_species_dose + temp + sym_state  + N_species_dose*temp + N_species_dose*sym_state + temp*sym_state, data= pam_all_nitrate.data) # model with all 9 variables and possible combinations

nullModel = lm(log_pam_percent_change ~  1, data= pam_all_nitrate.data) # model with the intercept only
...
summary(fullModel)
#model it selected for:
lm_pam_1 <- lm(formula = log_pam_percent_change ~ temp, data = pam_all_nitrate.data)
summary(lm_pam_1)

#lets check out some other models
lm_pam_2 <- lm(formula = log_pam_percent_change ~ temp + temp*sym_state, data = pam_all_nitrate.data)
summary(lm_pam_2)

lm_pam_3 <- lm(formula = log_pam_percent_change ~ temp + temp*sym_state + N_species_dose*sym_state, data = pam_all_nitrate.data)
summary(lm_pam_3)


AIC(lm_pam_1, lm_pam_2, lm_pam_3)
#Theyre SO close in AIC, let's go with #3 cause it is more informative :)

summary(emmeans(lm_pam_3, specs=pairwise ~ temp))
#0.1584
summary(emmeans(lm_pam_3, specs=pairwise ~ N_species_dose))
#nothing sig
summary(emmeans(lm_pam_3, specs=pairwise ~ sym_state))
#0.3255
summary(emmeans(lm_pam_3, specs=pairwise ~ temp*sym_state))
 # contrast            estimate    SE  df t.ratio p.value
 # temp20 A - temp30 A   0.4300 0.183 440   2.352  0.0882
 # temp20 A - temp20 S   0.3748 0.189 440   1.984  0.1956
 # temp20 A - temp30 S   0.3087 0.183 440   1.691  0.3297
 # temp30 A - temp20 S  -0.0552 0.182 440  -0.304  0.9903
 # temp30 A - temp30 S  -0.1213 0.175 440  -0.693  0.8998
 # temp20 S - temp30 S  -0.0661 0.181 440  -0.364  0.9834
summary(emmeans(lm_pam_3, specs=pairwise ~ temp*N_species_dose))
# nothing significant
...
lm_sym_1 <- lm(log_pam_percent_change ~  N_species_dose, data= all_sym)
summary(lm_sym_1)

#temp does not matter but dose does! Interesting
#WINNER
lm_sym_2 <- lm(log_pam_percent_change ~ temp + N_species_dose*temp, data= all_sym)
summary(lm_sym_2)
...
emm_sym_pam_N_species_dose_temp=emmeans(lm_sym_1, specs=pairwise ~ N_species_dose)
...
lm_apo_2 <- lm(log_pam_percent_change ~  N_species_dose*temp, data= all_apo)
summary(lm_apo_2)
#nothing really going on

lm_apo_3 <- lm(log_pam_percent_change ~  control_or_nutrient, data= all_apo)
summary(lm_apo_3)
