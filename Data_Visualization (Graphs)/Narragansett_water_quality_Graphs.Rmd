...
#Get rid of 2022 because that is when we did our experiment
...
gg_sum_month_year_wq_narra_nitrate <-ggplot(data = sum_month_year_wq_narra_nitrate ,
 ...
 geom_line(linetype="dotted", size=1, position = position_dodge(width = 0.45))+
    ...
    ggtitle (" ") +
  theme_classic() +
  theme(legend.position = "top",  # Keep legend in second plot
...
```{r adding nitrate to phosphate ratio}
narragansett_water_qual.data$nitrate_phosphate_ratio <- narragansett_water_qual.data$nitrate.uml/ narragansett_water_qual.data$orthophosphate.uml

hist(narragansett_water_qual.data$nitrate_phosphate_ratio)

range(na.omit(narragansett_water_qual.data$nitrate_phosphate_ratio))

range(na.omit(water_qual_bigelow_nit_phos_subset$nitrate_phosphate_ratio))
...
#max summer temp 
max(narragansett_water_qual.data_SUMMER$temperature.celsius)
```

```{r}
# Calculate the percentage of observations below 6 μM
percentage_below_6 <- sum(narragansett_water_qual.data$nitrate.uml < 6, na.rm=T) / nrow(narragansett_water_qual.data) * 100
