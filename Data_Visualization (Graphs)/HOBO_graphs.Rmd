...
```{r input data}
env_chamber_HOBO <- read.csv("/Data/Environmental_Chamber_Summer2022_9_5_22.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
...
temperate_HOBO <- read.csv("/Users/carolineianniello/Astrangia_Ecotox/Data/Temperate_System_Rotjan_Lab_9_5_22.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
temperate_HOBO$date<-as.factor(temperate_HOBO$date)
# its okay to omit rows with NAs
temperate_HOBO<-na.omit(temperate_HOBO)


all_HOBO <- read.csv("/Users/carolineianniello/Astrangia_Ecotox/Data/HOBO_all_by_day_4_7_23.csv", header=TRUE, sep=",",stringsAsFactors = FALSE)
...
gg_by_run<-ggplot(NULL, 
  ...
  theme_classic() +
    ...
    scale_color_manual("Treatment", labels=c("20°C Ambient","30°C Heat Challenge"), values=c("dodgerblue", "darkred")) +
   ylim(18, 30) +
     geom_line(data=sum_temp_by_run_chamber, color='darkred', linewidth=1) + 
      geom_line(data=sum_temp_by_run_temperate, color='dodgerblue', linewidth=1) +
     geom_errorbar(data=sum_temp_by_run_chamber, aes(ymin=temp-se, ymax=temp+se), width=0.5, size = 1, color="darkred") +
     geom_errorbar(data=sum_temp_by_run_temperate, aes(ymin=temp-se, ymax=temp+se), width=0.5, size = 1, color="dodgerblue") +
...
#subset so we can make the geomlines 
all_HOBO_by_day_chamber <- subset(all_HOBO_by_day, all_HOBO_by_day$chamber_or_temperate=="chamber")
sum_light_by_run_chamber <- summarySE(all_HOBO_by_day_chamber, measurevar = "light_intensity", groupvars = c("exp_day"))
all_HOBO_by_day_temperate <- subset(all_HOBO_by_day, all_HOBO_by_day$chamber_or_temperate=="temperate")
sum_light_by_run_temperate <- summarySE(all_HOBO_by_day_temperate, measurevar = "light_intensity", groupvars = c("exp_day"))
...
sum_light_by_run <- summarySE(all_HOBO_by_day, measurevar = "light_intensity", groupvars = c("exp_day", "chamber_or_temperate"))
gg_by_run_light<-ggplot(NULL, 
                        aes(x = exp_day,
           y = light_intensity, group=1))+
  theme(legend.position = "top",
          legend.text= element_text(size=14),
          legend.background = element_blank())+
  theme_classic() +
    theme(legend.position = "right",
          ...
          axis.text = element_text(size = 16),
          text = element_text(family = "serif", size = 16)) +
        geom_point(data = sum_light_by_run_chamber, size=4, aes(color="30°C Heat Challenge")) +
      geom_point(data = sum_light_by_run_temperate, size=4, aes(color="20°C Ambient"))+
    scale_color_manual("Treatment", labels=c("20°C Ambient","30°C Heat Challenge"), values=c("dodgerblue", "darkred")) +
 #  ylim(18, 30) +
     geom_line(data=sum_light_by_run_chamber, color='darkred', linewidth=1) + 
      geom_line(data=sum_light_by_run_temperate, color='dodgerblue', linewidth=1) +
     geom_errorbar(data=sum_light_by_run_chamber, aes(ymin=light_intensity-se, ymax=light_intensity+se), width=0.5, size = 1, color="darkred") +
     geom_errorbar(data=sum_light_by_run_temperate, aes(ymin=light_intensity-se, ymax=light_intensity+se), width=0.5, size = 1, color="dodgerblue") +
    labs(x = "Day of experiment", 
...
t.test(all_HOBO_by_day_chamber$light_intensity, all_HOBO_by_day_temperate$light_intensity)

range(all_HOBO_by_day_chamber$light_intensity)
range(all_HOBO_by_day_temperate$light_intensity)

range(all_HOBO_by_day_chamber$temp)
range(all_HOBO_by_day_temperate$temp)
t.test(all_HOBO_by_day_chamber$temp, all_HOBO_by_day_temperate$temp)
```
